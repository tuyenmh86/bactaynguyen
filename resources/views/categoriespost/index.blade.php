            ...
            <tbody>
                @foreach($categoriespost as $key => $category)
                    <tr>
                        ...
                        <td>
                            <label class="switch">
                                <input onchange="update_footer(this)" value="{{ $category->id }}" type="checkbox" <?php if($category->footer == 1) echo "checked";?> >
                                <span class="slider round"></span></label>
                        </td>
                        <td>
                            <label class="switch">
...
@section('script')
    <script type="text/javascript">
        function update_footer(el){
            ...
            $.post('{{ route('postcategory.updateFooter') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
