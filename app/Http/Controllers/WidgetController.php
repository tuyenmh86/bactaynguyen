...
class WidgetController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $widget->alias = '['.to_slug($request->name).']';
...
//            ImageOptimizer::optimize(base_path('public/').$product->featured_img);
        ...
        if($widget->save()){
            flash(__('Wiget đã được lưu thành công'))->success();
...
//            }
    ...
    public function update(Request $request, $id)
    {
        $widget = Widget::findOrFail($id);
        $widget->name = $request->name;
        // $widget->alias = to_sl$request->alias;
        $widget->description = $request->description;
        $widget->content = $request->content;
...
//            ImageOptimizer::optimize(base_path('public/').$product->featured_img);
        ...
        if($widget->save()){
            flash(__('widget has been inserted successfully'))->success();
//            if(Auth::user()->user_type == 'admin'){
                return redirect()->route('widgets');
//            }
//            else{
...
//            }
    ...
    public function destroy($id)
    {
        ...
        if(Widget::destroy($id)){
            ...
            flash(__('Widget has been deleted successfully'))->success();
            return redirect()->route('widgets');
