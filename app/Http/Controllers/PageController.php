...
//            ImageOptimizer::optimize(base_path('public/').$product->featured_img);
        ...
        if($request->hasFile('background')){
            $page->background = $request->background->store('uploads/page/background');
//            ImageOptimizer::optimize(base_path('public/').$product->featured_img);
        }
    ...
    public function update(Request $request, $id)
    {
       $page = Page::findOrFail($id);
        ...
        if($request->hasFile('featured_img')){
            $page->featured_image = $request->featured_img->store('uploads/page/image');
//            ImageOptimizer::optimize(base_path('public/').$product->featured_img);
        }
        if($request->hasFile('background')){
            $page->background = $request->background->store('uploads/page/background');
//            ImageOptimizer::optimize(base_path('public/').$product->featured_img);
        ...
        if($page->save()){
            flash(__('ProdPageuct has been inserted successfully'))->success();
            return redirect()->route('pages');
        }
        else{
            flash(__('Something went wrong'))->error();
